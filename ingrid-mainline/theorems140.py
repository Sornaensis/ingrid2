...
class Theorem50(Theorem):
    ...
    def run(self, ingrid_obj):
        ...
        if (forest == True) or (edges_Max != 'undt' and (edges_Max<=nodes_Min-(numOfComponents_Min))) and (nodes_Max != 'undt' and numOfComponents_Max != 'undt' and (edges_Min>=nodes_Max-(numOfComponents_Max))) or (arboricity_Max==arboricity_Min and (arboricity_Min==1.0)) or (girth_Min == 'undt') or (circumference_Min == 'undt'):
            ingrid_obj.set('forest', True)
            nodes = ingrid_obj.get('nodes', ind='Max')
            numOfComponents = ingrid_obj.get('numOfComponents', ind='Min')
            if nodes != 'undt':
