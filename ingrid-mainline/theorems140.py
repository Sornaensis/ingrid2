...
class Theorem304(Theorem):
    def __init__(self):
        super(Theorem304, self).__init__(304, "let x = mindeg*((mindeg+3.0)/2.0)-(1.0);\nif regular and mindeg >= 3.0 and edgeConnec >= mindeg-(2.0) and even nodes then \n{\n    edgeInd >= (nodes-(2.0*((nodes+1.0)/(2.0*x))))/2.0\n}\nelse if regular and mindeg >= 3.0 and edgeConnec >= mindeg-(2.0) and odd nodes then \n{\n    edgeInd >= (nodes-(maximum(2.0*((nodes+1.0+x)/(2.0*x)), 1.0)))/2.0\n};\n", "")
    def involves(self, str_invar):
    ...
    def run(self):
        ...
        if get("regular") == True and (minb("mindeg") != 'undt' and minb("mindeg") >= 3.0) and (minb("edgeConnec") != 'undt' and maxb("mindeg") != 'undt' and minb("edgeConnec") >= maxb("mindeg")-(2.0)) and evenInvar("nodes"):
            if minb("nodes") != 'undt' and minb("mindeg") != 'undt':
                try:
                    set("edgeInd",  (maxb("nodes")-(2.0*((maxb("nodes")+1.0)/(2.0*minb("mindeg")*((minb("mindeg")+3.0)/2.0)-(1.0)))))/2.0, ind='Min')
                except:
                    pass
            if minb("edgeInd") != 'undt' and minb("mindeg") != 'undt':
                try:
                    set("nodes",  2.0*(minb("edgeInd")*minb("mindeg")**2.0+3.0*minb("edgeInd")*minb("mindeg")-(minb("edgeInd"))+1.0)/(minb("mindeg")**2.0+3.0*minb("mindeg")-(3.0)), ind='Min')
                except:
                    pass
            if maxb("edgeInd") != 'undt' and maxb("nodes") != 'undt':
                try:
                    set("mindeg",  (-(6.0*minb("edgeInd"))+3.0*maxb("nodes")+sqrt(-((2.0*minb("edgeInd")-(maxb("nodes")))*(-(26.0*minb("edgeInd"))+21.0*maxb("nodes")+8.0))))/(2.0*(2.0*minb("edgeInd")-(maxb("nodes")))), ind='Max')
                except:
                    pass
            ...
            if minb("nodes") != 'undt' and minb("mindeg") != 'undt':
                try:
                    set("edgeInd",  (maxb("nodes")-(maximum(2.0*((maxb("nodes")+1.0+minb("mindeg")*((minb("mindeg")+3.0)/2.0)-(1.0))/(2.0*minb("mindeg")*((minb("mindeg")+3.0)/2.0)-(1.0))), 1.0)))/2.0, ind='Min')
                except:
                    pass
            if maxb("nodes") != 'undt' and maxb("mindeg") != 'undt' and maxb("edgeInd") != 'undt':
                try:
                    set("nodes",  maximum(2.0*((maxb("nodes")+1.0+minb("mindeg")*((minb("mindeg")+3.0)/2.0)-(1.0))/(2.0*minb("mindeg")*((minb("mindeg")+3.0)/2.0)-(1.0))), 1.0)+2.0*maxb("edgeInd"), ind='Max')
                except:
                    pass
        return
...
class Theorem322(Theorem):
    ...
    def run(self):
        ...
        oddInvar = self.oddInvar
        congruent = self.congruent
        if get("connected") == True and get("regular") == True and (maxb("mindeg") != 'undt' and maxb("mindeg") <= 4.0) and get("complete") == False and (((minb("nodes") != 'undt' and minb("nodes") >= 7.0) and (maxb("nodes") != 'undt' and maxb("nodes") <= 7.0)) or congruent("nodes", 3.0, 5.0)) and (((minb("nodes") != 'undt' and minb("nodes") > 13.0) or (maxb("nodes") != 'undt' and maxb("nodes") < 13.0)) and ((minb("nodes") != 'undt' and minb("nodes") > 18.0) or (maxb("nodes") != 'undt' and maxb("nodes") < 18.0))):
            if minb("nodes") != 'undt':
                try:
                    set("edgeCliqueCover",  floor((3.0*minb("nodes")+4.0)/5.0)+1.0, ind='Min')
                except:
                    pass
        elif get("connected") == True and get("regular") == True and (maxb("mindeg") != 'undt' and maxb("mindeg") <= 4.0) and get("complete") == False:
            if minb("nodes") != 'undt':
                try:
...
class Theorem330(Theorem):
    def __init__(self):
        super(Theorem330, self).__init__(330, "if edges >= maximum(((nodes-(mindeg))*(nodes-(mindeg)-(1.0)))/2.0+mindeg*mindeg+1.0, (floor((nodes+2.0)/2.0)*(floor((nodes+2.0)/2.0)-(1.0)))/2.0+floor((nodes-(1.0))/2.0)**2.0+1.0) then \n{\n    hamiltonian\n};\n", "")
    def involves(self, str_invar):
    ...
    def run(self):
        ...
        oddInvar = self.oddInvar
        congruent = self.congruent
        if (minb("edges") != 'undt' and maxb("nodes") != 'undt' and maxb("mindeg") != 'undt' and minb("edges") >= maximum(((maxb("nodes")-(minb("mindeg")))*(maxb("nodes")-(minb("mindeg"))-(1.0)))/2.0+minb("mindeg")*minb("mindeg")+1.0, (floor((maxb("nodes")+2.0)/2.0)*(floor((maxb("nodes")+2.0)/2.0)-(1.0)))/2.0+floor((maxb("nodes")-(1.0))/2.0)**2.0+1.0)):
            set("hamiltonian", True)
        return
class Theorem331(Theorem):
    ...
    def run(self):
        ...
        if minb("mindeg") != 'undt' and minb("nodeConnec") != 'undt' and minb("edges") != 'undt':
            try:
                set("nodes",  maxb("mindeg")-(minb("nodeConnec"))+sqrt(8.0*minb("edges")-(4.0*maxb("mindeg")**2.0)-(4.0*maxb("mindeg"))+4.0*minb("nodeConnec")**2.0-(4.0*minb("nodeConnec"))+1.0)/2.0+3.0/2.0, ind='Min')
            except:
                pass
    ...
    def run(self):
        ...
        if get("connected") == True and (minb("mindeg") != 'undt' and minb("mindeg") >= 3.0) and (minb("girth") != 'undt' and minb("girth") >= 5.0):
            ...
            if maxb("diameter") != 'undt' and maxb("domination") != 'undt' and maxb("nodes") != 'undt':
                try:
                    set("mindeg",  -(maxb("diameter")/3.0)+sqrt(4.0*maxb("diameter")**2.0-(12.0*maxb("diameter"))-(144.0*minb("domination"))+72.0*maxb("nodes")-(63.0))/6.0+3.0/2.0, ind='Max')
                except:
                    pass
        return
class Theorem335(Theorem):
    def __init__(self):
        super(Theorem335, self).__init__(335, "if bandwidth >= nodes/2.0 then \n{\n    nosolve edges >= (2.0*floor(nodes/2.0)-(1.0))*(nodes/(nodes-(2.0)))**(bandwidth-(floor(nodes/2.0)))\n};\n", "")
    def involves(self, str_invar):
    ...
    def run(self):
        ...
        oddInvar = self.oddInvar
        congruent = self.congruent
        if (minb("bandwidth") != 'undt' and maxb("nodes") != 'undt' and minb("bandwidth") >= maxb("nodes")/2.0):
            if minb("nodes") != 'undt' and minb("bandwidth") != 'undt':
                try:
                    set("edges",  (2.0*floor("nodes"/2.0)-(1.0))*("nodes"/("nodes"-(2.0)))**(minb("bandwidth")-(floor("nodes"/2.0))), ind='Min')
                except:
                    pass
        return
        ...
        if (minb("girth") != 'undt' and minb("girth") >= 5.0):
            if maxb("nodes") != 'undt' and maxb("mindeg") != 'undt' and maxb("edges") != 'undt':
                try:
                    ...
                    pass
            if minb("domination") != 'undt' and minb("mindeg") != 'undt' and minb("edges") != 'undt':
                try:
